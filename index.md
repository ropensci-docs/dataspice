...
![CRAN
Version](https://www.r-pkg.org/badges/version/dataspice)![CI](https://github.com/ropensci/dataspice/workflows/R-CMD-check/badge.svg)[![Codecov
test
...
- Create a helpful dataset README webpage for your data similar to how
  [pkgdown](https://pkgdown.r-lib.org/) creates websites for R packages.
...
dataspice](reference/figures/dataspice_workflow.png)
...
creates template metadata spreadsheets in a folder (by default created
in the `data` folder in the current working directory).
...
the associated helper function and/or [Shiny](https://shiny.posit.co/)
app.

#### Helper functions

- [`prep_attributes()`](https://docs.ropensci.org/dataspice/reference/prep_attributes.md)
  populates the **`fileName`** and **`variableName`** columns of the
  `attributes.csv` file using the header row of the data files.

- [`prep_access()`](https://docs.ropensci.org/dataspice/reference/prep_access.md)
  populates the **`fileName`**, **`name`** and **`encodingFormat`**
  columns of the `access.csv` file from the files in the folder
  containing the data.

To see an example of how
[`prep_attributes()`](https://docs.ropensci.org/dataspice/reference/prep_attributes.md)
works, load the data files that ship with the package:
...
this function can be applied over multiple files to populate the header
names
...
- [`edit_attributes()`](https://docs.ropensci.org/dataspice/reference/edit_attributes.md)
  opens a Shiny app that can be used to edit `attributes.csv`. The Shiny
  app displays the current `attributes` table and lets the user fill in
  an informative description and units (e.g. meters, hectares, etc.) for
  each variable.
- [`edit_access()`](https://docs.ropensci.org/dataspice/reference/edit_access.md)
  opens an editable version of `access.csv`
- [`edit_creators()`](https://docs.ropensci.org/dataspice/reference/edit_creators.md)
  opens an editable version of `creators.csv`
- [`edit_biblio()`](https://docs.ropensci.org/dataspice/reference/edit_biblio.md)
  opens an editable version of `biblio.csv`

![edit_attributes Shiny app](reference/figures/edit_attributes.png)
...
generates a json-ld file (“linked data”) to aid in [dataset
...
file](reference/figures/listviewer.png)
...
- [`build_site()`](https://docs.ropensci.org/dataspice/reference/build_site.md)
  creates a bare-bones `index.html` file in the repository `docs` folder
...
![dataspice-website](reference/figures/website_example.png)
...
the validation errors produced by
[`EML::eml_validate`](https://docs.ropensci.org/emld/reference/eml_validate.html)
and fix those. Note that this will likely require familiarity with the
[EML Schema](https://eml.ecoinformatics.org/) and the [EML
...
- [ISO 19139](https://www.iso.org/standard/32557.html) - Geographic Info
  Metadata XML schema
